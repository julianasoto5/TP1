...
extern QueueHandle_t colaJuego;
extern QueueHandle_t colaFeedback;
extern QueueHandle_t colaComunicacion;
...
static void TimerCallbackJuego(TimerHandle_t xTimer)
{
    ...
    xQueueSendFromISR(colaJuego, &evento, &xHigherPriorityTaskWoken);
...
void Tarea_Juego(void *pvParameters)
{
    ...
    while(1) {
            ...
            case STATE_IDLE:
                ...
                if (xQueueReceive(colaJuego, &evento, portMAX_DELAY)) {
            ...
            case STATE_READY:
                ...
                xQueueSend(colaMovimiento, &inicio, 0)
                xQueueSend(colaFeedback, &inicio, 0);
            ...
            case STATE_PLAYING:
                if (xQueueReceive(colaJuego, &evento, portMAX_DELAY)) {
                    if (evento.tipo == DISPARO) {
                        ...
                        xQueueSend(colaFeedback, &msg, 0);
                        //Enviar puntaje actualizado a la tarea de comunicación
                        xQueueSend(colaComunicacion, &msg, 0);
                    }
            ...
            case STATE_GAME_OVER:
                ...
                xQueueSend(colaFeedback, &fin, 0);
                xQueueSend(colaComunicacion, &fin, 0);
                // Enviar STOP a Tarea Movimiento 
                xQueueSend(colaMovimiento, &fin, 0);

                if (xQueueReceive(colaJuego, &evento, portMAX_DELAY)) {
                    if (evento.tipo == RESET) {
                        xQueueReset(colaJuego);
